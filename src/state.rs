...
/// `Task` picks up the output by marking the task as closed.
///
...
/// consumed by the `Task`. A task becomes closed in the following cases:
///
...
///
/// The `Task` is a special case in that it is only tracked by this flag, while
/// all other task references (`Runnable` and `Waker`s) are tracked by the
/// reference count.
...
///
/// This flag is set when notifying the awaiter. If an awaiter is concurrently
/// registered and notified, whichever side came first will take over the
/// reposibility of resolving the race.
...
///
/// Note that the reference counter only tracks the `Runnable` and `Waker`s. The
/// `Task` is tracked separately by the `TASK` flag.
